...
### In Scope
- Migrate `src/update_check.rs:fetch_latest_version()` from blocking client on a `std::thread` to async client on `tokio::spawn`
...
- Add error context to HTTP error types (timeout, connection, redirect) so failures are diagnosable
...
**Target**: `tokio::spawn(check_and_notify())` → async reqwest on Tokio task.

The `spawn()` function is called once at startup from `main.rs:run()`. Converting the background task to use `tokio::spawn` is the cleanest path — no runtime detection needed since `main.rs` has no Tokio runtime at all. A new Tokio runtime will be created for this task only.

Implementation: replace `thread::spawn` with `tokio::spawn` and make `check_and_notify` async. The `fetch_latest_version` function becomes `async fn fetch_latest_version()` using `reqwest::Client` (not blocking). Add a `tokio::runtime::Runtime` scoped to the spawn call.
...
The test function becomes `async fn` with `#[tokio::test]`. The blocking client builder is replaced with `reqwest::Client::builder()`. Retry logic stays the same. The test gate `RUN_E2E=1` remains.

#### 4. `Cargo.toml`
After all three conversions verify green, remove `"blocking"` from the reqwest features:
```toml
# Before
...
|------|--------|-------------|
| `src/update_check.rs` | Modified | `spawn()` → `tokio::spawn`; `fetch_latest_version` becomes async |
...
| `Cargo.toml` | Modified | Remove `blocking` feature from reqwest |
...
- `tokio` runtime features: verify `rt-multi-thread` remains (already present; needed for `tokio::spawn`)
...
- [ ] `cargo test --all-features` passes (including `RUN_E2E=1` catalog integrity test)
