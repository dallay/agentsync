...
`./target/debug/agentsync skill --project-root <tmp-with-package-json-and-gh-workflow> suggest --json`
✅ emitted `node_typescript` and `github_actions`
...
1. `design.md` still documents earlier phase-2 flag naming that no longer matches the implemented
   CLI surface.
...
The JSON identifier mismatch is fixed, the previously missing runtime scenarios now have passing
automated coverage, and the full change verifies successfully; only non-blocking design-document
drift remains.
