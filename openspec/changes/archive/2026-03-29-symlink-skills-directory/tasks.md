...
- [x] 4.2 Verify fallback behavior: confirm that when directory symlink creation is not permitted (e.g., restricted Windows without developer mode), `agentsync apply` exits non-zero and the error output includes the target destination path. Reference: `src/linker.rs` `create_symlink()` error path for context. To reproduce on Windows: run without elevated privileges on a filesystem that restricts symlinks. On Unix: make destination parent read-only (`chmod 555`). Manual verification step — no automated test added for OS-level restrictions. Note: Windows may surface raw OS error text instead of the exact phrase "Failed to create symlink".
