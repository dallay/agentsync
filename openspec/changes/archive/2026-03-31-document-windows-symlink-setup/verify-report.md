...
**Compliance summary**: 0/14 scenarios have automated scenario-level test coverage; 14/14 scenarios
are manually satisfied by the implemented docs and successful docs build.
...
- No automated docs-specific scenario tests or link-check suite back the spec scenarios;
  verification is based on successful docs build plus manual content review.
- Workspace contains unrelated untracked local directories (`.agents/command/`, `.claude/`) that are
  outside this change and may warrant cleanup/confirmation before archiving if a clean working tree
  is expected.

**SUGGESTION** (nice to have):

- Consider adding a lightweight docs link/content verification check in CI for future
...
The documentation change matches the proposal/spec/design/tasks, the new Windows guide is
discoverable and appropriately scoped, and the docs build passes; archive can proceed, but only with
warnings because scenario-level automated verification is absent.
