...
3. The init wizard copies skills from all agent skill directories into `.agents/skills/`, skipping collisions with a warning.
...
7. `scan_agent_files()` detects skill directories for all 8 agents (Cursor, Codex, Gemini, OpenCode, Roo, Factory, Vibe, Antigravity) when they exist with content, and ignores them when empty or absent.
8. `scan_agent_files()` detects command directories for Claude, Gemini, and OpenCode when they exist with content.
9. `scan_agent_files()` detects `.windsurfrules`, `OPENCODE.md`, and `AMPCODE.md` as instruction files.
10. `scan_agent_files()` detects 9 agent-specific MCP config files when present.
11. The wizard migrates command directories into `.agents/commands/` with collision detection.
12. The wizard notes all detected MCP configs with migration suggestion messages without parsing or copying them.
13. `init()` creates `.agents/commands/` alongside `.agents/skills/`.
14. `DEFAULT_CONFIG` includes `[agents.claude.targets.commands]` with correct source, destination, and type.
15. `DEFAULT_CONFIG` includes `gemini` and `opencode` agent sections with instructions and skills targets.
16. `DEFAULT_CONFIG` remains valid TOML that parses into a `Config` struct.
17. New tests cover each new scan entry, migration path, collision scenario, and config parsing.
