...
`<PLUGIN@MARKETPLACE>` spelling for `add`, `update`, and `remove`.
...
Git sources are snapshotted into AgentSync-owned project state so later `apply`, `status`, and
`dry-run` operations remain offline.

#### `update`

Resolve the configured reference again and replace the lock entry after validation.
This is also the only operation, besides `add`, that may refresh a Git source snapshot.
...
Validate locked source content and report drift without materializing or executing anything.
