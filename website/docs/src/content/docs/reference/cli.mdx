...
Runs diagnostics and health checks on your AgentSync configuration and environment.
...
**Checks performed**:
- Validates the `agentsync.toml` file.
- Checks for duplicate or overlapping symlink destinations.
- Audits `.gitignore` for correct markers and entries.
- Verifies executability of MCP server commands.
