...
`skill suggest` is read-only by default. `--install` opens an interactive selector in a TTY, while `--install --all` is the TTY-safe non-interactive path. Recommendation-driven installs reuse the same install pipeline and registry updates as `skill install`, and they skip recommendations that are already installed.
