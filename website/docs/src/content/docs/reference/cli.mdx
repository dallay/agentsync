...
3. Built-in default configuration (6 agents enabled: claude, copilot, cursor, codex, gemini, opencode; claude-desktop included but disabled by default)
...
For the full team decision guide, see <a href="/guides/gitignore-team-workflows/">Gitignore Team Workflows</a>. For install-time and Git hook automation patterns around `apply`, see <a href="/guides/git-hook-automation/">Git Hook Automation</a>.

If `apply` fails because the local Windows environment cannot create symlinks yet, fix the shell/setup prerequisites first with the <a href="/guides/windows-symlink-setup/">Windows Symlink Setup guide</a>.
...
- `--offline`: Zero network for the invocation, including the update checker. Missing Git plugin snapshots fail; run `agentsync plugin restore` first. `--dry-run` never restores snapshots either, but may still run the update checker.
...
<CommandTabs command="apply --agents gemini" />

Runs only gemini (CLI overrides config).

<CommandTabs command="apply --agents copilot,claude,gemini" />
...
<CommandTabs command="apply --dry-run" />

Skip gitignore reconciliation for this run only:

<CommandTabs command="apply --no-gitignore" />

Preview without any gitignore writes or cleanup:

<CommandTabs command="apply --dry-run --no-gitignore" />
...
> [!NOTE]
> The `list` command is currently not implemented.
...
Manage repository-owned, vendor-neutral plugin materialization.
...
Plugin selections accept either `<MARKETPLACE/PLUGIN>` or the vendor-style
`<PLUGIN@MARKETPLACE>` spelling for `add`, `update`, `remove`, and `restore`.

#### `add`

Resolve the configured marketplace reference, validate the selected plugin, write
`.agents/plugins.lock.toml`, and materialize supported skills.
...
Resolve the configured reference again and replace the lock entry after validation.
...
anything. Missing Git snapshots fail; status never restores them. Plugin MCP declarations are
reported as `ALLOWED` or `PENDING`; `ALLOWED` means approved by `plugins.allowed_mcp`, not that
MCP will necessarily be written when global MCP or agent filters disable it. Raw `${ENV}`
references are reported literally and never expanded.

```bash
agentsync plugin status [--json]
```

#### `restore`

Fetch the exact locked Git commit for selected plugins. Does not resolve `marketplace.reference`
and does not write `plugins.lock.toml` or `agentsync.toml`. With no argument, restores every
plugin in `[plugins.selections]`. Local sources are skipped.

```bash
agentsync plugin restore [MARKETPLACE/PLUGIN] [--json]
```

Online `agentsync apply` restores a **missing** snapshot automatically. It does not repair an
invalid existing snapshot; use `plugin restore` for that.

#### `remove`

Remove only skills recorded as owned by the selected plugin, remove its lock entry, and remove the
matching selection from `agentsync.toml`. It also revokes all `allowed_mcp` approvals under
`plugin/<marketplace>/<plugin>/` atomically. Re-adding the plugin starts its MCP servers as
`PENDING`.
...
- JSON for CI:
  - <CommandTabs command="status --json" />
