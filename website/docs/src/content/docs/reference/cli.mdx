...
8. Optionally create backups of original files (after user confirmation, only for files that were copied)
9. For each migrated **skills** target, explicitly ask whether it should use `symlink` or `symlink-contents`, recommend `symlink`, and preserve an already-correct directory symlink by default
10. Generate a configuration (`.agents/agentsync.toml`) that manages these files with symlinks
11. Run a post-init validation summary that warns if a selected skills target is configured with `symlink-contents` while the destination is already a directory symlink to `.agents/skills`
