...
## Repository-owned Plugins

AgentSync can materialize the supported, vendor-neutral parts of a plugin into the canonical
`.agents/skills/` directory and merge standard `.mcp.json` declarations into the existing MCP
pipeline. It does not install vendor caches, enable plugins in user configuration, or execute
hooks, scripts, binaries, LSPs, or MCP servers.

```toml
[plugins]
enabled = true
lockfile = "plugins.lock.toml"
# Default-deny: only explicitly approved plugin MCP servers are exported.
...
- `source` accepts a relative local checkout or an HTTPS GitHub repository.
- `reference` is used only by explicit `agentsync plugin add` and `agentsync plugin update`.
- `plugins.lock.toml` records the immutable revision, content hash, selected skills, MCP servers,
  and provenance. `agentsync apply` and `agentsync plugin restore` never resolve a new revision.
- `allowed_mcp` is a default-deny approval list using names such as
  `plugin/internal/engineering/safe-fixture`. It approves a logical server identity, not a
  revision; an explicit plugin update can therefore change the approved definition and must be
  reviewed. Entries must be unique and use exactly four identifier segments.
- Git snapshots live under `.agents/.agentsync-plugin-sources`. Online `apply` restores a missing
  snapshot from the locked SHA. `apply --offline`, `apply --dry-run`, and `plugin status` never
  download; recover with `agentsync plugin restore`.
- Vendor marketplace files are declaration/discovery inputs; they do not silently activate Claude or
  Codex plugins.
- The first supported plugin layout is `skills/<skill-id>/SKILL.md` plus a root `.mcp.json`.
- Plugin MCP declarations are treated as executable configuration. Values for credential-like
  environment variables and headers (`TOKEN`, `SECRET`, `PASSWORD`, `API_KEY`, `Authorization`,
  and similar names) must use an exact local environment reference such as `${TOKEN}` rather than
  literal credentials. Authorization headers may use the complete `Bearer ${TOKEN}` template;
  AgentSync rejects literal prefixes and suffixes before fan-out.
- ZIP downloads are bounded both by compressed download size and by extracted entry count/size.
  These limits protect a clone or `plugin add` from archive bombs, but do not replace source review
  or vendor trust.
- Private GitHub resolution may use `GITHUB_TOKEN` from the process environment. The token is sent
  only to GitHub requests and is never written to the lockfile, source snapshot, or generated config.

Use `agentsync plugin status` to validate the lock and source without changing files. Use
`agentsync plugin restore` to recover or repair Git snapshots from the lockfile.
...
> **See also:** <BaseLink href="/concepts/sync-types/">Sync Types</BaseLink> for the full semantics
> of `symlink`, `symlink-contents`, `nested-glob`, and `module-map` targets, including patterns,
> placeholders, and filename resolution.
