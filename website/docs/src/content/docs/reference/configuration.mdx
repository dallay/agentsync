...
- `plugins.lock.toml` records the immutable revision, content hash, selected skills, MCP servers,
  and provenance. `agentsync apply` requires the lockfile and never resolves a new revision.
- Git add/update operations also keep an AgentSync-owned source snapshot under
  `.agents/.agentsync-plugin-sources`; apply, status, and dry-run read that snapshot only and fail
...
Use `agentsync plugin status` to validate the lock and source without changing files.
