...
AgentSync supports a canonical `.agents/commands/` directory for agent commands (slash commands, markdown-based prompts, etc.). Commands are synced to agent-specific locations using `symlink-contents`, the same way skills are managed.
