...
AgentSync relies on a tight integration between Rust and TypeScript:

1. **Core Development**: Most logic resides in `src/` (Rust).
2. **NPM Distribution**: `npm/agentsync/` contains the logic for finding and executing the correct platform-specific Rust binary.
3. **Shared Interfaces**: While the stacks are separate, the CLI interface (arguments and flags) must remain consistent. Always verify changes with `make verify-all`.
