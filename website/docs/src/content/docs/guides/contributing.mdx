...
3. **Verify your changes**:
   - For Rust: run `cargo test`, `cargo fmt`, and (optionally) `cargo clippy`.
   - For TypeScript: run `pnpm run typecheck` and `pnpm run build` to ensure types and the distribution build succeed.
