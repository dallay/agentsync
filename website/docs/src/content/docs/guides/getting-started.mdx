...
> **Note:** The `command -v` check ensures the install process does not fail if `agentsync` is not yet installed (e.g., on a fresh clone or in CI), while still reporting actual synchronization errors when the tool is present.
